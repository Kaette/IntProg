...
public class GUI {
    ...
    private GUI(){
        ...
        mainFrame.setJMenuBar(createMenuBar()); //Adds JMenuBar
    ...
    public JMenuBar createMenuBar(){
        ...
        //Funktionalitet for newGame knappen
        ...
        abortItem.setAccelerator(KeyStroke.getKeyStroke(
        KeyEvent.VK_A, CTRL_SHIFT ));
        ...
        playLogItem.setAccelerator(KeyStroke.getKeyStroke(
        KeyEvent.VK_L, CTRL_SHIFT ));
        playLogItem.addActionListener( e -> playLogButton.doClick());
        
        JMenuItem saveLogItem = new JMenuItem("Save Log...");
        logMenu.add(saveLogItem);
        saveLogItem.setAccelerator(KeyStroke.getKeyStroke(
        KeyEvent.VK_S, CTRL_SHIFT ));
    ...
    private void applyOptions() {
        ...
        //Toll size & robbery
        int tollSize, riskRob, minRob, maxRob = 0;
        try{
            tollSize = Integer.parseInt(tollSizeTextField.getText());
            riskRob  = Integer.parseInt(robberyTextField.getText());
            minRob = Integer.parseInt(minLossTextField.getText());
            maxRob = Integer.parseInt(maxLossTextField.getText());
            ...
            if(minRob < 0 || maxRob < minRob || minRob > 100 || maxRob > 100){
                JOptionPane.showMessageDialog(mainFrame, "Min. loss and Max. loss must be between 0 and 100, and Max. loss cannot be lower than Min. loss", "Malformed input", JOptionPane.ERROR_MESSAGE);
                return;
            }
        } catch (NumberFormatException e){
            JOptionPane.showMessageDialog(mainFrame, "All values must be integers.", "Malformed input", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        game.getSettings().setRisk(riskRob);
        game.getSettings().setTollToBePaid(tollSize);
        game.getSettings().setMinMaxRobbery(minRob, maxRob);
    ...
    public JFrame createOptionsDialogBox(){
        ...
        //Min loss
        JLabel minLabel = new JLabel("Min. loss when robbed");
        tollAndRobberyPanel.add(minLabel);
        
        minLossTextField = new JTextField("10", 10);
        tollAndRobberyPanel.add(minLossTextField);
        
        JLabel moneyMin = new JLabel("€ in [0,100]");
        tollAndRobberyPanel.add(moneyMin);
        ...
        tollAndRobberyPanel.add(moneyMax);
